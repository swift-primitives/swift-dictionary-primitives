...
//
// [TEST-017] Model-based testing: Compare behavior against reference implementation
// to verify invariants across many operations.

@Suite("Dictionary.Ordered.Model")
struct DictionaryOrderedModelTests {
    @Suite struct Unit {}
...
private struct ArrayDictModel<Key: Hashable, Value: Equatable> {
    var keys: [Key] = []
    var dict: [Key: Value] = [:]
...
extension DictionaryOrderedModelTests.Unit {

    @Test
    func `Insertion order preserved`() {
        var rng = LCG(seed: 22222)
        var orderedDict = Dictionary<Int, String>.Ordered()
    ...
    func `Iteration matches model`() {
        ...
        var orderedDict = Dictionary<Int, Int>.Ordered()
...
extension DictionaryOrderedModelTests.EdgeCase {
    ...
    func `Empty dictionary operations`() {
        let empty = Dictionary<String, Int>.Ordered()
...
extension DictionaryOrderedModelTests.Integration {
