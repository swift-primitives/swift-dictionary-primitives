...
// Note: Dictionary.Ordered.Small is unconditionally ~Copyable (inline storage requires deinit),
...
extension Dictionary_Primitives_Core.Dictionary.Ordered.Small where Value: Copyable {
    /// Iterator for Dictionary.Ordered.Small key-value pairs.
    ///
...
extension Dictionary_Primitives_Core.Dictionary.Ordered.Small.Iterator: Sendable
...
extension Dictionary_Primitives_Core.Dictionary.Ordered.Small: Sequence.`Protocol` where Value: Copyable {
    ...
    public borrowing func makeIterator() -> Iterator {
        var keySnapshot = Buffer<Key>.Linear(minimumCapacity: count)
        var valueSnapshot = Buffer<Value>.Linear(minimumCapacity: count.retag(Value.self))
        var i: Index_Primitives.Index<Key> = .zero
        let end = count.map(Ordinal.init)
        while i < end {
            if let heapKeys = _heapKeys {
                keySnapshot.append(heapKeys[i])
            } else {
                keySnapshot.append(_inlineKeys[i])
            }
